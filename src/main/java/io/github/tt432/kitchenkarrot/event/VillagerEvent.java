...
public class VillagerEvent {
    ...
    public static void addVillagerTrades(VillagerTradesEvent event) {
        ...
        if (ModCommonConfigs.FARMER_BUY_GEM_CARROT.get()
        ...
        if (ModCommonConfigs.BUTCHER_SELL_SALT.get()
                && event.getType() == VillagerProfession.BUTCHER) {
            trades.get(1)
                    .add(
                            (p1, p2) ->
                                    new MerchantOffer(
        ...
        if (ModCommonConfigs.BUTCHER_SELL_OIL.get()
                && event.getType() == VillagerProfession.BUTCHER) {
            trades.get(1)
                    .add(
                            (p1, p2) ->
